/**
 * LuxChat Full Version - Complete Multi-User Application
...
class LuxChatFull {
    ...
    init() {
        console.log('🎉 LuxChat Full Version initializing...');
    ...
    setupApp() {
        ...
        console.log('✅ LuxChat Full Version ready!');
    }
    
    restoreUserSession() {
        const savedUser = localStorage.getItem('luxchat_current_user');
    ...
    broadcastMessage(message) {
        ...
        const storageKey = `luxchat_message_${message.roomCode}_${Date.now()}`;
        const broadcastData = {
    ...
    handleTyping() {
        ...
        this.typingTimeout = setTimeout(() => {            // Stop typing after 1 second of no input
        }, 1000);
    }
    ...
    setupTheme() {
        ...
        if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
            if (!localStorage.getItem('luxchat_settings')) {
        ...
        if (window.matchMedia) {
            window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (e) => {
                if (!localStorage.getItem('luxchat_theme_manual')) {
    ...
    toggleTheme() {
        ...
        localStorage.setItem('luxchat_theme_manual', 'true');
    ...
    setupMessageBroadcasting() {
        ...
        window.addEventListener('storage', (e) => {
            if (e.key && e.key.startsWith('luxchat_message_')) {
        ...
        if ('BroadcastChannel' in window && this.roomManager?.currentRoom) {
            this.messageChannel = new BroadcastChannel(`luxchat_messages_${this.roomManager.currentRoom.code}`);
    ...
    saveSettings() {
        try {
            localStorage.setItem('luxchat_settings', JSON.stringify(this.settings));
        } catch (error) {
    ...
    loadSettings() {
        try {
            const saved = localStorage.getItem('luxchat_settings');
            if (saved) {
    ...
    saveMessageHistory() {
        try {
            ...
            localStorage.setItem('luxchat_message_history', JSON.stringify(messagesToSave));
        } catch (error) {
    ...
    loadMessageHistory() {
        try {
            const saved = localStorage.getItem('luxchat_message_history');
            if (saved) {
    ...
    setupGlobalChat() {
        ...
        if (!globalRoom) {
            ...
            const globalRoomCode = this.generateGlobalRoomCode();
...
document.addEventListener('DOMContentLoaded', () => {
    window.luxChat = new LuxChatFull();
});

// Handle page unload
window.addEventListener('beforeunload', () => {
    if (window.luxChat) {
        window.luxChat.destroy();
    }
});

// Global access for debugging
window.LuxChatFull = LuxChatFull;
