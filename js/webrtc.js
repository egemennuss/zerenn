/**
 * WebRTC Manager for Zeren - GitHub Pages Compatible
...
class WebRTCManager {
    ...
    async initializeRoomWithDiscovery(roomId) {
        ...
        const participantKey = `zeren_participant_${roomId}_${this.userId}`;
    ...
    getRoomParticipants(roomId) {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key && key.startsWith(`zeren_participant_${roomId}_`)) {
                try {
    ...
    setupBroadcastChannel(roomId) {
        ...
        if ('BroadcastChannel' in window) {
            this.broadcastChannel = new BroadcastChannel(`zeren_${roomId}`);
    ...
    setupStorageListener(roomId) {
        ...
        window.addEventListener('storage', (e) => {
            if (e.key && e.key.startsWith(`zeren_participant_${roomId}_`)) {
    ...
    announcePresence(roomId) {
        ...
        const announcementKey = `zeren_announcement_${roomId}_${Date.now()}`;
    ...
    startHeartbeat(roomId) {
        this.heartbeatInterval = setInterval(() => {
            // Update our timestamp to show we're still active            const participantKey = `zeren_participant_${roomId}_${this.userId}`;
    ...
    leaveRoom() {        // Clean up our presence
        if (this.roomId) {
            const participantKey = `zeren_participant_${this.roomId}_${this.userId}`;
